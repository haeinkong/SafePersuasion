...
def get_gpt_answer(system_prompt, user_prompt):
    load_dotenv()
...
def run_gpt(task, prompt_type):
    ...
    for idx, row in tqdm(
        ...
        answer = get_gpt_answer(system_prompt, user_prompt)    
        preds.append(answer)
    ...
    output_path = os.path.join(RESULTS_DIR, f"{task}-gpt-{prompt_type}.csv")
